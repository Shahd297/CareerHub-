...
export const geminiService = {
  async generateDailyTask(specialization: Specialization, level: number, lang: 'ar' | 'en') {
    const prompt = `Act as an expert in ${specialization}. Generate a realistic work task for a level ${level} student on CareerHub platform. Slogan: Experience work before you work. Include title, detailed description, and target skill. Language: ${lang === 'ar' ? 'Arabic' : 'English'}.`;
  ...
  async analyzeTaskSubmission(specialization: Specialization, task: string, submission: string, lang: 'ar' | 'en') {
    const prompt = `You are an AI Mentor for CareerHub specialized in ${specialization}. Slogan: Experience work before you work. Evaluate this student submission for the task: "${task}".
  ...
  async generateAssessment(specialization: Specialization, lang: 'ar' | 'en') {
    const prompt = `Create a 10-question placement test for CareerHub platform in ${specialization}. 
  ...
  async chatWithMentor(specialization: Specialization, message: string, lang: 'ar' | 'en') {
    const prompt = `You are an AI Mentor for a platform called CareerHub. Slogan: Experience work before you work. Your specialty is ${specialization}. 
